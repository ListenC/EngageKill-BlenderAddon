...
updater.addon = "apply_blendshapes"
...
def update_settings_ui(self, context, element=None):
    ...
    settings = get_user_preferences(context)
    if not settings:
        box.label(text="Error getting updater preferences 1", icon='ERROR')
        return
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    settings = get_user_preferences(context)
    if not settings:
        row.label(text=f"Error getting updater preferences 3 {settings}", icon='ERROR')
        return
...
def register(bl_info):
    ...
    updater.user = "ListenC"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "EngageKill-BlenderAddon"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/ListenC/EngageKill-BlenderAddon/"
